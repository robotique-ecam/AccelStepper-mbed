...
bool AccelStepper::runSpeed() {
  ...
  // Gymnastics to detect wrapping of either the nextStepTime and/or the current
  // time
...
void AccelStepper::setOutputPins(uint8_t mask) {
  ...
  //        (LOW ^ _pinInverted[i]));
  *_pin0 =
      (mask & (1 << 0)) ? (HIGH ^ _pinInverted[0]) : (LOW ^ _pinInverted[0]);
  *_pin1 =
      (mask & (1 << 1)) ? (HIGH ^ _pinInverted[1]) : (LOW ^ _pinInverted[1]);
  if (_interface == FULL4WIRE || _interface == HALF4WIRE) {
    *_pin2 =
        (mask & (1 << 2)) ? (HIGH ^ _pinInverted[2]) : (LOW ^ _pinInverted[2]);
    *_pin3 =
        (mask & (1 << 3)) ? (HIGH ^ _pinInverted[3]) : (LOW ^ _pinInverted[3]);
  } else if (_interface == FULL3WIRE || _interface == HALF3WIRE)
    *_pin2 =
        (mask & (1 << 2)) ? (HIGH ^ _pinInverted[2]) : (LOW ^ _pinInverted[2]);
}
